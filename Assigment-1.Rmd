...
First i join the two graphs friendship and advice. 
...
from <- recode(var = edge_aggr$from, recodes = "node_attributes$ID=node_attributes$DEPT")
...
2 graphs (friends and advice)

Stats:
size, color, shape, alpha, text

node: color: diff degree (advice: asked or asker, friend: reprocity of friendship)
node: size: centrality (eigen or betweenness)
node: shape: level of hierarchy

edge: arrow/alpha for direction, green for both ways, and red for one way

I calculate degree dif. 
```{r}
g_advice_in <- degree(g_advice, mode = c("in"))
g_advice_out <- degree(g_advice, mode = c("out"))
g_advice_dif <- g_advice_in-g_advice_out
g_advice_dif

g_friendship_in <- degree(g_friendship, mode = c("in"))
g_friendship_out <- degree(g_friendship, mode = c("out"))
g_friendship_dif <- g_friendship_in-g_friendship_out
g_friendship_dif
```

I calculate centrality. 
```{r}
g_advice_cent <- g_advice %N>%
  mutate(centrality_eigen = centrality_eigen(directed = TRUE, scale = TRUE), 
         centrality_between = centrality_betweenness(directed = TRUE), 
         g_advice_dif = g_advice_dif)
...
g_advice_cent %>% 
  ...
  geom_node_point(aes(size = centrality_eigen,
                    color = g_advice_dif, # Notice that
...
g_friendship_cent %>% 
  ...
  geom_node_point(aes(size = centrality_eigen,
                    color = g_friendship_dif, # Notice that
                    shape = as.factor(LEVEL)),
                  show.legend = TRUE) +
  scale_colour_gradient(low = "red", high = "green") +
  theme_graph() +
